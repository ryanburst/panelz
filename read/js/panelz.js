...
$(document).ready(function () {
    ...
    var Settings = function (_EventClass) {
        _inherits(Settings, _EventClass);
        ...
        function Settings() {
            ...
            _this.localSettings = _this.getLocalSettings();
            _this.loadConfig($.extend({}, _this.DEFAULTS, _this.getUserSettings()));
            _this.setFields();
    ...
    var Tutorial = function (_EventClass2) {
        _inherits(Tutorial, _EventClass2);

        function Tutorial(settings) {
            _classCallCheck(this, Tutorial);

            var _this2 = _possibleConstructorReturn(this, (Tutorial.__proto__ || Object.getPrototypeOf(Tutorial)).call(this));

            _this2.settings = settings;

            _this2.interactable = new Hammer.Manager($('.tutorial')[0]);
            _this2.interactable.add(new Hammer.Swipe());

            _this2.addEventListeners();

            if (_this2.settings.get('showTutorial')) {
                _this2.show();
            }
            return _this2;
        }
    ...
    var Page = function (_EventClass3) {
        ...
        function Page(config) {
            ...
            _this3.panels = [];
            _this3.PANEL_ANIMATION_SPEED = ViewPort.settings.get('panelTransitions');
            _this3.SHOW_PAGE_ON_ENTER = ViewPort.settings.get('showPageOnEnter');
            _this3.SHOW_PAGE_ON_EXIT = ViewPort.settings.get('showPageOnExit');
